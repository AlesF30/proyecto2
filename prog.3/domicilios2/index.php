        ...
        <script>
            ...
            function CargarProvincias(id_pais) {
                ...
                if (id_pais != 0) {
                    ...
                    $.post("control.php",   /* 1er parametro: ARCHIVO PHP DEL SERVIDOR */

                           {function: 'LeerProvincias',   /* 2do parametro: DATOS A ENVIAR */
                            id_pais: id_pais}, 

                            function (data) {   /* 3er parametro: FUNCION RESULTADO DE LA CONEXION */
                                if (data != 0) {
                                    datos_provincias = JSON.parse(data);   //El json en texto plano, se convierte en OBJETO json

                                    for (let i=0; i<datos_provincias.length; i++) {           
                                        let pcia_id     = datos_provincias[i]['id_provincia'];
                                        let pcia_desc   = datos_provincias[i]['descripcion'];

                                        nuevaOpcion = new Option(pcia_desc, pcia_id);
